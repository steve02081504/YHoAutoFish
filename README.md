...
面向《异环》的 Windows 桌面自动钓鱼工具：自动抛竿、自动上钩、自动溜鱼、自动结算、自动记录战绩。
  ...
  <p>
    ...
    <img alt="Version" src="https://img.shields.io/badge/version-1.2.4-63E4E4?style=flat-square">
  </p>
</div>
...
1.2.4 是基于 1.2.3 的稳定性更新，重点完善结算关闭确认、鱼饵不足提示判定和月卡复位流程，并继续保持 1600 × 900、1080p、2K 等常见分辨率下的自动购买鱼饵适配。

- 新增月卡每日复位功能，可在设置中开启，按北京时间每日 05:02 执行一次双 ESC 复位。
- 修复成功结算已记录后重复 ESC 可能退出钓鱼初始界面的问题，避免结算后进入错误上级界面导致卡死。
- 优化鱼饵不足提示判定，只有在抛竿后明确看到黑底鱼饵不足提示时才进入自动购买流程。
- 保留鱼饵不足自动购买万能鱼饵流程：检测到鱼饵不足后进入钓鱼商店，购买设置数量的万能鱼饵后继续钓鱼。
- 自动购买数量按 99 的倍数限制，设置页会提示所需鱼鳞币数量。
- 修复钓鱼商店会缓存上次选中商品时，可能直接购买错误鱼饵的问题。
- 优化无上限万能鱼饵商品定位，必须结合完整商品卡与同卡货币特征确认。
- 修复 1600 × 900 和 2K 下正确商品详情已出现但被 OCR 噪声拒绝的问题。
- 优化购买确认弹窗和获得物品弹窗识别，减少购买流程中断。
- 修复购买完成退出商店后误触发额外 ESC 的问题。
- 优化鱼饵不足短时提示检测，减少正常抛竿时误进入购买流程。
- 新增鱼饵商店与详情页调试开关，便于定位不同分辨率下的识别问题。
- 优化标题栏群入口与赞助入口展示，赞助显示可在设置中关闭或重新开启。
- 保留 1.2.3 对鱼饵商店识别、购买确认弹窗、详情页确认、赞助入口和调试开关的稳定性修复。

完整发布说明见 [release_notes_v1.2.4.md](release_notes_v1.2.4.md)。

## 下载与运行

推荐使用 GitHub Release 中的压缩包，不需要自己配置 Python 环境。

1. 下载 `YHoAutoFish-v1.2.4-windows.zip`。
...
1.2.4 的默认参数已按当前游戏 A/D 移动速度调校。多数用户不需要修改。
...
Gitee 国内源使用连续分卷附件，例如 `YHoAutoFish-v1.2.4-windows.zip.001`、`.002`、`.003`。程序会自动下载所有分卷、校验 SHA256、合并为完整更新包并安装，普通用户不需要手动合并分卷。若下载过程中点击取消，程序会清理未完成的分卷、合并包和临时目录。
...
普通用户只需要下载 `YHoAutoFish-v1.2.4-windows.zip`，解压后运行 `YHoAutoFish.exe`。
