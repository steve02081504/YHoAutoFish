...
面向《异环》的 Windows 桌面自动钓鱼工具：自动抛竿、自动上钩、自动溜鱼、自动结算、自动记录战绩。
  ...
  <p>
    ...
    <img alt="Version" src="https://img.shields.io/badge/version-1.2.3-63E4E4?style=flat-square">
  </p>
</div>
...
1.2.3 是基于 1.2.2 的稳定性更新，重点完善鱼饵不足后的自动购买流程，并加强 1600 × 900、1080p、2K 等常见分辨率下的商品识别与详情确认。

- 新增鱼饵不足自动购买万能鱼饵流程：检测到鱼饵不足后进入钓鱼商店，购买设置数量的万能鱼饵后继续钓鱼。
...
- 保留 1.2.2 对结算误判、溜鱼识别、用户接管保护和在线更新的稳定性修复。

完整发布说明见 [release_notes_v1.2.3.md](release_notes_v1.2.3.md)。

## 下载与运行

推荐使用 GitHub Release 中的压缩包，不需要自己配置 Python 环境。

1. 下载 `YHoAutoFish-v1.2.3-windows.zip`。
...
1.2.3 的默认参数已按当前游戏 A/D 移动速度调校。多数用户不需要修改。
...
Gitee 国内源使用连续分卷附件，例如 `YHoAutoFish-v1.2.3-windows.zip.001`、`.002`、`.003`。程序会自动下载所有分卷、校验 SHA256、合并为完整更新包并安装，普通用户不需要手动合并分卷。若下载过程中点击取消，程序会清理未完成的分卷、合并包和临时目录。
...
普通用户只需要下载 `YHoAutoFish-v1.2.3-windows.zip`，解压后运行 `YHoAutoFish.exe`。
